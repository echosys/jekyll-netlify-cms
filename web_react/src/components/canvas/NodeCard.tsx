...
const NodeCard = memo(({ data }: NodeProps) => {
  ...
  useEffect(() => {
    if (!node.profile_image_ref) { setImgUrl(null); return; }
    // profile_image_ref = "resources/<filename>"
    const filename = node.profile_image_ref.replace(/^resources\//, '');

    // Find resource by filename to get its real id (needed for IndexedDB key)
    const resource = resources?.find(
      (r) => r.filename === filename || r.original_filename === filename,
    );
    const resourceId = resource?.id ?? filename; // fallback to filename (fs mode ignores this)

    getImageUrl(folderName, resourceId, filename).then((url) => {
      setImgUrl(url);
    });
