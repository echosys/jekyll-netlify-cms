...
export default function TagEditor({ resources: resourceList, initialIndex, onClose }: Props) {
  ...
  const initialResource = resourceList[listIndex] ?? resourceList[0];
  ...
  // snapshot of what was last saved/loaded so we can detect edits
  ...
  const switchTo = useCallback((newIdx: number) => {
    ...
    const next = resourceList[newIdx];
    setListIndex(newIdx);
    setResource(next);
    setImgUrl(null);
  ...
  }, [resourceList, checkDirty, resource, filename, date, location, customTags, updateResource]);
  ...
  const handleClose = useCallback(() => {
    ...
    onClose();
  }, [checkDirty, resource, filename, date, location, customTags, updateResource, onClose]);
  ...
  useEffect(() => {
    const onMove = (e: MouseEvent) => {
      ...
      setDrawRect({ x: Math.min(s.x, pos.x), y: Math.min(s.y, pos.y), w: Math.abs(pos.x - s.x), h: Math.abs(pos.y - s.y) });
    };
    const onUp = (e: MouseEvent) => {
      ...
      drawStartRef.current = null;
      if (!pos || !s) return;
      const rect: Rect = { x: Math.min(s.x, pos.x), y: Math.min(s.y, pos.y), w: Math.abs(pos.x - s.x), h: Math.abs(pos.y - s.y) };
      setDrawRect(null);
  ...
  const handleSaveTag = () => {
    ...
    if (tagUseAsProfile && !resolvedNodeId.startsWith('__orphan__')) {
      ...
      if (treeNode) updateNode({ ...treeNode, profile_image_ref: `resources/${resource.filename}` });
    }
    setPendingRect(null);
    setSelectedRegionIdx(null);
    setPage('normal');
              ...
              <div style={{ flex: 1, padding: 16, overflowY: 'auto', display: 'flex', flexDirection: 'column', gap: 12 }}>
                ...
                {resource.regions.map((reg, i) => (
                  <div key={i} style={{ display: 'flex', alignItems: 'center', gap: 6, padding: '4px 0', borderBottom: '1px solid #f0f0f0' }}>
                    ...
                    <button onClick={() => { setResource((prev) => ({ ...prev, regions: prev.regions.filter((_, idx) => idx !== i) })); isDirty.current = true; }}
                ...
                <button onClick={handleSaveMeta}
                  style={{ marginTop: 'auto', padding: '9px 0', borderRadius: 7, border: 'none', background: '#1565C0', color: '#fff', fontWeight: 700, cursor: 'pointer' }}>
                  💾 Save & Close
                </button>
              </div>
            ) : (
