...
import { listUsers, setAllowedTrees } from '../../db/realtimeDbAdapter';
import { listTrees } from '../../db/storageAdapter';
...
export default function UserTreeAccessDialog({ onClose }: Props) {
    ...
    (async () => {
      ...
      if (user) {
        try {
          ...
          setUsers(fetched.map((u) => ({ username: u.username, role: u.role as 'dev' | 'user', color: u.color ?? '#888', allowed_trees: u.allowed_trees ?? [] })));
      ...
      try {
        ...
        if (attempt !== loadAttemptRef.current) return;
        setAllTrees(all.map((t) => t.treeName));
      } catch (e: any) {
  ...
  const handleSave = async () => {
    ...
    setSuccessMsg('');
    try {
      await setAllowedTrees(selectedUser, editedTrees);
      setUsers((prev) => prev.map((u) => u.username === selectedUser ? { ...u, allowed_trees: editedTrees } : u));
