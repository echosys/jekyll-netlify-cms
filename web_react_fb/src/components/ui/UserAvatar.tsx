...
export default function UserAvatar({ user, onlineUsers, onLogout, onManageAccess, onUpdateColor }: Props) {
              ...
              {AVATAR_COLORS.map((c) => (
                <button
                  ...
                  style={{
                    ...
                    border: user.color === c ? '2px solid #333' : '1px solid rgba(0,0,0,0.1)',
                    cursor: 'pointer', padding: 0, boxSizing: 'border-box',
                    boxShadow: user.color === c ? '0 0 0 1px #fff' : 'none'
                  }}
                />
              ))}
            </div>
